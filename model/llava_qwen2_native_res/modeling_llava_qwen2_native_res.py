...
from model.llava_qwen2_native_res.configuration_llava_qwen2_native_res import LlavaQwen2NativeResConfig
from model.projectors import Identity


class LlavaQwen2NativeResForCausalLM(PreTrainedModel):
    config_class = LlavaQwen2NativeResConfig
    main_input_name = "inputs_embeds"
    base_model_prefix = "language_model"
    _no_split_modules = ["Qwen2_5_VLVisionTransformer", "Qwen2DecoderLayer"]
    _supports_flash_attn_2 = True
    supports_gradient_checkpointing = True

    def __init__(
        self,
        config: LlavaQwen2NativeResConfig,
        ...
        if vision_model is not None:
            ...
            if "qwen2_5_vl" in config.vision_config.model_type:
        ...
        self.projector = Identity()  # Patch_merger already in vision_model
        ...
        for i, patch_indices_list in enumerate(patch_indices_list_batch):
            ...
            if patch_indices_list is not None and len(patch_indices_list) > 0 and sample_image_features is not None:
                ...
                inputs_embeds[i] = self._merge_inputs_embeds_with_image_features(
                    obj_features, inputs_embeds[i].unsqueeze(0), input_ids[i].unsqueeze(0)
                )
            # Current sample contains caption only
            elif sample_image_features is not None and self.image_token_id in input_ids[i]:
                inputs_embeds[i] = self._merge_inputs_embeds_with_image_features(
    ...
    def forward(
        ...
        **kwargs,
        ...
        if pixel_values is not None:
            ...
            image_features = self.vision_model(pixel_values.to(inputs_embeds.dtype), grid_thw=image_grid_thw)
            ...
            if (
                ...
                inputs_embeds = self._merge_inputs_embeds_with_mmcs_obj_features(
                    image_features, inputs_embeds, input_ids, patch_indices_list_batch, num_image_tiles_batch
                )
